...
// One row per requirement: status glyph + text; evidence lives in a tooltip.
// Missing skill requirements offer one-tap "+ I have this" (adds the skill).

import { Check, Circle, Plus, X } from "lucide-react";
import { useT } from "@/lib/i18n/LanguageProvider";
import type { JdRequirement, RequirementCoverage } from "@/lib/optimizer/types";
...
export function CoverageMatrix({
  ...
  return (
    <div className="rounded-xl border border-stone-200 overflow-hidden">
      <div className="px-3.5 py-2.5 bg-stone-50 border-b border-stone-100 text-sm font-semibold text-brand-navy">
        {t("Job requirements")}
      </div>
      <ul className="divide-y divide-stone-100">
        {requirements.map((req) => {
          ...
          return (
            <li key={req.id} className="flex items-start gap-2.5 px-3.5 py-2.5" title={tooltip}>
              {status === "strong" ? (
              ...
              <div className="min-w-0 flex-1">
                <span
                  className={`text-sm leading-snug ${
                    status === "missing" ? "text-stone-700" : "text-stone-600"
                  }`}
                >
                  {req.text}
                </span>

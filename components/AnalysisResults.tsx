...
export function AnalysisResults({ results }: AnalysisResultsProps) {
  const [activeTab, setActiveTab] = useState<"overview" | "changes" | "optimized">("overview");
  ...
  return (
    <div className="bg-white/5 rounded-3xl border border-white/10 overflow-hidden backdrop-blur shadow-[0_30px_90px_rgba(0,0,0,0.45)]">
      {/* Header with Score */}
      <div className="bg-gradient-to-r from-blue-600/90 to-indigo-600/90 p-6 text-white">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-xl font-bold mb-1">Screening Analysis</h3>
            <p className="text-white/85">{results.summary}</p>
          </div>
          <div className="text-center">
            <div className={`w-20 h-20 rounded-2xl border ${scoreTone} backdrop-blur flex items-center justify-center`}>
              <span className="text-3xl font-bold text-white">{results.overallScore}</span>
            </div>
            <p className="text-sm mt-2 text-white/85">Screening Score</p>
            <p className="text-xs text-white/70">Est. chance to pass initial screen</p>
          </div>
        </div>
      </div>
              ...
              <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
                ...
                <div className="flex flex-wrap gap-2">
                  {results.keywords.present.map((keyword, index) => (
                    <span key={index} className="px-3 py-1 bg-emerald-400/15 text-emerald-100 border border-emerald-300/20 rounded-full text-sm">
                      {keyword}
                    </span>
                  ))}
                </div>
              ...
              <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
                ...
                <div className="flex flex-wrap gap-2">
                  {results.keywords.missing.map((keyword, index) => (
                    <span key={index} className="px-3 py-1 bg-rose-400/15 text-rose-100 border border-rose-300/20 rounded-full text-sm">
                      {keyword}
                    </span>
                  ))}
                </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {getLearningLinks(skill).map((link) => (
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="text-sm px-3 py-1 rounded-full bg-indigo-500/20 text-indigo-100 border border-indigo-300/20 hover:bg-indigo-500/30 transition-colors"
                            >
                              {link.label}
                            </a>
                          ))}

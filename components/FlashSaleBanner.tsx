...
export function FlashSaleBanner() {
  ...
  return (
    <div className="fixed inset-x-0 bottom-4 z-[60] px-4 pointer-events-none">
      <div className="pointer-events-auto mx-auto max-w-xl rounded-sm bg-white text-brand-navy shadow-[0_24px_60px_-20px_rgba(184,134,11,0.55)] border border-brand-gold/60 overflow-hidden">
        ...
        <div className="flex items-center gap-3 sm:gap-4 px-4 py-3">
          <span className="grid place-items-center h-10 w-10 rounded-sm bg-brand-gold text-white flex-shrink-0">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-xs text-stone-500 mt-0.5">
              <span className="font-semibold text-brand-navy">{fmtPrice(price)}</span>{" "}
              <span className="line-through text-stone-400">{fmtPrice(anchor)}</span> · {t("{credits} credits · ends in", { credits })}{" "}
              <span className="font-semibold tabular-nums text-brand-gold">{remaining}</span>
            </div>
          </div>
          <button
            type="button"
          ...
          <button
            ...
            className="flex-shrink-0 grid place-items-center h-8 w-8 rounded-sm text-stone-400 hover:text-brand-navy hover:bg-stone-100 transition-colors"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
